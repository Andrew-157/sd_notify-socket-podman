...
int main() {
    ...
    while (1) {
        ...
        sleep(sleep_time);
        sd_notify(0, "WATCHDOG=1");  // NOTE: accoring to systemd docs, it is recommended to not check return value of the call to sd_notify: https://www.freedesktop.org/software/systemd/man/latest/sd_notify.html#Return%20Value
    }

	closelog();  // i don't know if it makes sense to call it here, would script ever get here even?
}
